...
void Maps::recursiveDivision(int xl, int xh, int yl, int yh, Eigen::MatrixXi &maze) {
  ...
  if (xl < xh - 3 &&
    ...
    while (valid == false) {
      xm = (std::rand() % (xh - xl - 1) + xl + 1); // generating random number between xl+1 and
                                                   // xh-1(pointless to add a wall at the sides)
      ym = (std::rand() % (yh - yl - 1) + yl + 1); // generating random number between yl+1 and
                                                   // yh-1(pointless to add a wall at the sides)
